...
const NewArrivals = () => {
  ...
  useEffect(() => {
    const fetchNewArrivals = async () => {
      try {
        ...
        console.log("New Arrivals data:", response.data); // Add this line to inspect data
      } catch (error) {
        console.error(error);
      }
    };
  ...
  const scroll = (direction) => {
    ...
    // Delay update so the scroll position updates after animation
  ...
  const updateScrollButtons = () => {
    ...
    if (container) {
      ...
      setCanScrollRight(leftScroll < maxScrollLeft - 1); // tolerance for rounding
    }
  };
